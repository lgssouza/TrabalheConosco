# Por que trabalhar na GAS Tecnologia?

"Por que você gostaria de trabalhar na GAS Tecnologia?" - essa é a pergunta clássica
...
# Líder de Mercado em segurança nas transações bancárias

A GAS Tecnologia é lider absoluta no mercado de segurança em transações eletrônicas.
São mais de 40 milhões de computadores que possuem  nosso produto instalado, protegendo o acesso 
a serviços de Internet Banking e instituições contra ataques de Hackers evitando que os mesmos 
consigam roubar credenciais sensíveis que podem causar muitos transtornos e prejuizos.
...
* Caso você tenha alguma restrição em relação a enviar o pull-request publicamente, não tem problema! Encaminhe o seu CV (ainda especificando a vaga) para [avaliacao.tecnica@gastecnologia.com.br](avaliacao.tecnica@gastecnologia.com.br).
...
Envie email para avaliacao.tecnica@gastecnologia.com.br com o seu CV e a área de interesse
...
* Área de Projetos de Infraestrutura (2 vagas)
...
* contribua, diariamente, para construção de uma __ambiente de trabalho descontraído e cheio de energia__;

* se __preocupe com seus colegas de trabalho__ e com o seu time;

* que você busque tornar o seu __trabalho mais eficiente__;

* que você __pense e repense__ sobre a sua __forma de trabalhar sua e da sua equipe__.
...
A maioria do nosso código está escrito em Python, temos muitas coisas em Javascript também, mas somos pragmáticos e gostamos de experimentar - por isso temos coisas em Go, Java, Objective C, Android, iOS entre outras. Todo o nosso ambiente de desenvolvimento e produção é fundamentado em cima de tecnologias open-source, começando pelo SO, Linux. Usamos Linux para tudo. Os times têm autonomia para escolher as tecnologias que julgarem mais adequadas para resolver cada problema, então com certeza temos muito mais coisas envolvidas nessa equação.
...
* Gostar de experimentar novas ideias e formas de trabalhar;
...
## Área de Projetos de Infraestrutura

A área de Projetos de Infraetsrutura desenha, testa e implementa as soluções de infra necessárias para os diversos projetos desenvolvidos na globo.com. Trabalhamos com tecnologias voltadas para aplicações web, com uma preocupação constante com robustez, escalabilidade e facilidade de manutenção. Tratamos todas as nossas aplicações como sistemas de missão crítica. Usamos software livre na grande maioria das aplicações e ferramentas por trás do portal.

**Requisitos**

Procuramos profissionais com experiência em:

* Instalação e configuração de servidores Linux.
* Instalação e configuração de serviços voltados para Internet como web servers (Apache, Nginx), proxies e caches (Squid, Varnish)
* Configuração de equipamentos de rede (switches, routers, load balancers).
* Instalação de servidores de aplicação/containers para aplicações Python, Ruby e Java.
* Programação em shell script e Perl.
* Configuração de aplicações para usar sistemas de banco de dados (MySQL, Oracle, MongoDB).

**Diferenciais**

Experiência nesses tópicos é um diferencial para os candidatos:

* Gestão de serviços de autenticação e autorização de acesso (PAM, LDAP).
* Uso de sistemas de gerência de configuração (Puppet, Chef).
* Uso de sistemas de monitoração e medidas de desempenho (Nagios, Cricket, statsd).
* Programação em Python, Ruby ou Java.

As vagas nessa equipe são na cidade do Rio de Janeiro.
